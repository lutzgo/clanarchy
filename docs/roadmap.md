...
|---|---|---|---|---|---|
| L1 | `LAN (1)` → Jellyfin `10.0.90.10:8096/tcp` | UDM-Pro ZBF policy `Allow Jellyfin to Services`, ID `10001` | Jellyfin is reached directly; there is no reverse proxy yet | **M5** — repoint clients at `jellyfin.<domain>` and replace with the permanent `LAN → traefik:443` rule | **removal pending — M5's manual step, [#86](https://github.com/lutzgo/clanarchy/pull/86).** The rule is on the UDM-Pro, so the repo cannot retire it; this row closes when lgo deletes the policy. Note the code half already makes it INERT: the container now refuses `8096` from anything but `10.0.90.12`, so from the moment M5 deploys this policy permits traffic the backend drops. Delete it anyway — a permit rule that does nothing is the kind of thing someone later "fixes" by removing the restriction instead |
| L2 | `IoT (20)` → Jellyfin `10.0.90.10:8096/tcp` | UDM-Pro ZBF policy `Allow Jellyfin to Services`, ID `10000` | TVs / streaming devices live on the IoT VLAN | **M5** — same as L1 | **removal pending — M5's manual step, [#86](https://github.com/lutzgo/clanarchy/pull/86).** Same as L1: inert on deploy, deleted by hand. This is the row with a household cost attached — the TV on VLAN 20 loses Jellyfin the moment M5 deploys and gets it back only once it is pointed at `jellyfin.goclan.org`. Do both in one window |
...
| — | `LAN (1)` + `IoT (20)` → traefik `10.0.90.12:80,443/tcp` | UDM-Pro ZBF, M5 | **This is the rule M5 exists to make possible** — one permanent rule per consumer zone, pointing at one address, replacing L1/L2 and pre-empting L4/L8 | **permanent** — every future web service is a Traefik route plus a Technitium record, and touches the UDM-Pro not at all | to be created by lgo with [#86](https://github.com/lutzgo/clanarchy/pull/86). Same shape as the qBittorrent and arr rows: source zone `Internal` narrowed to the listed networks, **ports in the Destination card** (the source card has a Port section too and it is the one you see first), `Auto Allow Return Traffic` ticked. **`80` is in the list on purpose**: Traefik's `:80` entryPoint serves nothing but a 308 to `:443`, and permitting only 443 would leave that listener unreachable — a redirect that exists in the config and cannot be hit is worse than not having one. If you would rather not permit 80, delete the `web` entryPoint and the port from `containers/traefik.nix` in the same change, so the two halves keep agreeing |
...
## M5 — `feat/ernst-traefik` (built — deploy + UDM-Pro pending)
