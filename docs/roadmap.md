...
### Still owed — all of it client-side
...
- [ ] Confirm `ncadmin` still logs in with its password. **This is the recovery
      path and it is the one thing above that OIDC working makes it tempting to
      skip.** `allow_multiple_user_backends=1` is what keeps the local form on
      the page; that it is set is not the same as having used it.
- [ ] An app password + `PROPFIND /remote.php/dav/files/<user>/`.
