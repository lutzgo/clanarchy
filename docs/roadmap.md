...
### The defect that took OIDC down, and why it did not look like one
...
- [ ] Browser: the Authelia provider → 2FA → lands back logged in **as the
      existing account**, not a second empty one. Then confirm `ncadmin` still
      logs in with its password. *The redirect out to Authelia is now proven
      (404 → 303, correct authorize URL); what is untested is the half AFTER
      the portal — the callback, the token exchange, and whether
      `preferred_username` maps onto an existing account rather than creating a
      second one.*
