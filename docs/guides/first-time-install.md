...
- **`miralda`** / **`jens`** — YubiKey-first. `modules/users/lgo.nix` declares `~/.config/sops/age/keys.txt` with the PIV identity stub via `home.file`, so sops can decrypt secrets encrypted to your recipient from first login — no manual step needed. See [YubiKey guide](yubikey.md#age-encryption-clan-vars--sops).
