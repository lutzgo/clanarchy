...
- `birte` — Steam Deck OLED (Galileo). Jovian-NixOS Steam Gaming Mode by default; "Switch to Desktop" drops into KDE Plasma 6 via SDDM. Built entirely against `nixpkgs-unstable` (Jovian only supports unstable). Fully on `main` — the `feat/birte-steamdeck` branch this used to live on merged in PR #4 (2026-07-12) and no longer exists.
...
| `containers/miniflux.nix` | Miniflux — the feed reader (M27). nspawn, `10.0.90.28`, local PostgreSQL on `/srv/state/miniflux`. Takes **no uid** (DynamicUser). It shipped as a LAN-only `appApiHosts` exemption and **moved to `protectedHosts` one day later** when off-LAN reading was wanted, so it is now public behind forward-auth **and** OIDC (ledger row L18) — the door got stronger, not weaker, and the cost is that no native RSS reader can ever connect to the name again. The fleet's first real Prometheus application target since M19 |
...
| `containers/music-assistant.nix` | Music Assistant — the player controller over Navidrome's library (M30). nspawn, `10.0.90.30`, uid/gid 3040 (upstream's `DynamicUser` forced **off**: `StateDirectory` + `DynamicUser` migrates the bind mount). Reads Navidrome over the Subsonic API (`opensubsonic`, **not** `filesystem_local` — no media mount, no second index) and scrobbles back. **The second container with a leg on VLAN 20**, because the `musiccast` provider is mDNS-discovery-only and the one speaker is a Yamaha receiver at `10.0.20.31`; its veth is `iot1`, since `iot0` is the hub's and the name is host-global. `protectedHosts`, **LAN-only**, no ledger row. The machine is `mass`, not `music-assistant` |
...
| `@clanarchy/local-ai` | Self-hosted LLM stack + OpenCode. **ernst has not run Ollama since M19** — llama-swap in front of `llama-server` took its place and its port (11434); the role is still named `ollama` for continuity. miralda keeps a real local Ollama (ROCm). llama-swap binds loopback only, so in-container consumers reach it through `exposeOn`: a per-consumer point-to-point /128 ULA with its own socket proxy and one firewall accept — monitoring (`fdca:fe90::`/`mon0`), Open WebUI (`fdca:fe91::`/`ai1`), Karakeep (`fdca:fe92::`/`ai2`), Home Assistant (`fdca:fe93::`/`ai3`). **The veth name is a host-global allocation, not a per-container one** — M27 and M19 both picked `ai0` and Open WebUI silently lost its leg for five days; `machines/ernst/networking.nix` now asserts uniqueness. `agent` (M29) is **mneme**, the household agent daemon: it speaks the *Ollama* wire protocol to Home Assistant — whose 2026.5.4 has no `llama_cpp` integration and an `openai_conversation` with no base-URL option — and plain OpenAI to everything else, injecting the constitution on the way through. `voice` (M29) adds `wyoming-faster-whisper` + `wyoming-piper` for Assist, which is a **second** STT on the machine (different protocol, different weight format from `speech`'s whisper.cpp — neither can serve the other). **M29b** gave the agent a git-versioned markdown wiki at `/srv/state/mneme/wiki` (uid **3039**; recall is *injected* every turn, not left to a tool call — see [docs/guides/agent-memory.md](docs/guides/agent-memory.md)), a `web_search` tool reaching SearXNG over a fifth ULA leg (`fdca:fe94::`/`web0`, pointing *into* a container rather than out of one), and a `generate_image` tool whose PNG lands in HA's `www/mneme` and **evicts the resident 21 GiB model on every call**. `opencode` reaches ernst over a restricted SSH port-forward rather than needing a local model | ollama role: miralda, ernst; agent + voice: ernst; opencode: miralda (local), jens (tunnelled to ernst) |
